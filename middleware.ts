...
export function middleware(req: NextRequest) {
  ...
  // Bypass static assets immediately
  ...
  if (pathname.startsWith('/api/')) {
    ...
    response.headers.set('X-RateLimit-Reset', String(Math.ceil(rl.reset / 1000)));
    
    // API Versioning header
    const versionMatch = pathname.match(/^\/api\/(v\d+)(?:\/|$)/);
    response.headers.set('X-API-Version', versionMatch ? versionMatch[1] : 'v2');
